...
- `.codex/skills/` and `.agents/skills/` map slash-style workflows (`/auto-debug`, `/auto-SDLC`, `/automatic`) for Codex CLI surfaces; they're documented in `.agents/slash-commands.md`. They don't affect Codex directly but explain the names you'll see referenced in user messages.
